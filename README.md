...
一個面向創作者與工作室的原型專案：先用多個模型批量生成情節，再用 LLM 做分群與摘要，讓使用者勾選感興趣的群組，最後再用多個模型把這些情節編織成完整故事，並以 Markdown 輸出。

### 亮點特性
- 多模型情節生成：第一次可手動挑選 5 個模型，或一鍵隨機。
- LLM 分群與摘要：用向量與聚類找相似情節，用 GPT/LLM 產生群組名稱與代表摘要。
- 使用者互動勾選：在 UI 以卡片/清單呈現群組，支援多選、預覽、合併/拆分。
- 多模型故事生成：根據已勾選的情節，挑選 3 個模型（或隨機）生成完整故事，以 Markdown 輸出。
- 可重現與控參：支援 seed、溫度、top_p、權重隨機、去重閾值等。
- 可擴充的模型連接：OpenAI、Anthropic、Google、Mistral、Azure OpenAI、Groq、Ollama（本地）。
